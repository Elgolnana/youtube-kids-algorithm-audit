...
WATCH_SECONDS = 300              # 오염용 영상 시청 시간(초) — 실제 실험 5분
...
def stage_analyze():
    ...
    try:
        ...
        # ── 결과 해석 ──
        story += [Spacer(1,0.8*cm), Paragraph("실험 결과 해석", h2_s)]

        interpretations = [
            ("1. 실험 전 기준선 (P1) — 모두 0%",
             "3개 프로필 모두 오염 전 홈피드·비디오페이지에서 부적합 영상 0%. "
             "새 계정으로 깨끗한 상태에서 실험이 시작됐음을 확인."),

            ("2. 오염 후 변화 (P1→P2)",
             "홈피드 기준 보통형 90%, 적극형 70%, 방관형 30%로 급증. "
             "보통형이 적극형보다 높은 것은 알고리즘의 개인별 추천 차이로 추정. "
             "비디오페이지는 중립 시드 영상 기반이라 오염 영향이 상대적으로 낮음."),

            ("3. AI 개입 효과 — 홈피드 (P2→P3)",
             "보통형(영상 단위 차단): 90% → 20% (70%p 감소). "
             "적극형(채널 단위 차단): 70% → 10% (60%p 감소). "
             "방관형(개입 없음): 30% → 20% (자연 감소)."),

            ("4. AI 개입 효과 — 비디오페이지 (P2→P3)",
             "적극형: 20% → 10% 감소 (채널 차단이 비디오페이지까지 효과). "
             "보통형: 10% → 40% 역설적 증가. 영상 단위 차단은 홈피드만 정화하며, "
             "차단된 영상의 채널이 살아있어 비디오페이지로 오염이 밀린 것으로 해석. "
             "방관형: 20% → 40% 증가. 개입 없이 시간이 지나면서 오염이 확산됨."),

            ("5. 핵심 결론",
             "채널 단위 차단(적극형)이 홈피드와 비디오페이지 모두에서 가장 효과적. "
             "영상 단위 차단(보통형)은 홈피드 정화 효과는 있으나 "
             "비디오페이지 오염이 오히려 증가하는 부작용이 발생. "
             "무개입(방관형)은 오염이 시간이 지날수록 피드 전반으로 확산됨."),
        ]

        for title, body in interpretations:
            story.append(Paragraph(title, ParagraphStyle("IH", fontName=kr_b, fontSize=11, spaceAfter=3, spaceBefore=8)))
            story.append(Paragraph(body, p_s))
